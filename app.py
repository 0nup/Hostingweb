...
def create_default_files(server_dir):
    ...
    if not os.path.exists(main_py):
        with open(main_py, 'w', encoding='utf-8') as f:
            f.write('''# ARFAT𖣘 HOSTING - Default Bot
import time

print("=" * 40)
print("Bot is running on ARFAT𖣘 HOSTING")
...
def api_create_server():
    ...
    if not username:
        username = f"IshrakArfat{random.randint(10000, 99999)}"
...
if __name__ == '__main__':
    print("\n" + "=" * 50)
    print("🚀 ARFAT𖣘 HOSTING - FINAL")
    print("=" * 50)
    port = os.environ.get('PORT', '5000')
    print(f"📍 Landing: http://127.0.0.1:{port}")
    print(f"📍 Admin: http://127.0.0.1:{port}/login")
    print(f"🔗 API: http://127.0.0.1:{port}/api/create")
